...
#
"""
Download all of the aperture (window) types from a PH-Navigator project and build them
into Honeybee-PH 'WindowUnitType' geometry and 'WindowConstruction' objects. Optionally
...
EM July 5, 2026
    Args:
        ...
        _download: (bool) Set True to download the aperture types from the project.

    Returns:
        window_types_: (CustomCollection[WindowUnitType]) The Honeybee-PH window-unit
            types (geometry), keyed by type-name. Wire to 'HBPH+ - Create Window Geometry'.
        ...
        install_types_: (CustomCollection[List[PhApertureInstallType]]) The per-edge
            ...
            left]. Wire to 'HBPH+ - PH-Nav Set Aperture Psi-Installs' to apply them to the
...
ghenv.Component.Name = "HBPH+ - PH-Nav Get Apertures"
DEV = honeybee_ph_plus_rhino._component_info_.set_component_params(ghenv, dev=False)
if DEV:
    from honeybee_ph_plus_rhino.gh_compo_io.ph_navigator.v1 import apertures_get as gh_compo_io
    reload(gh_compo_io)
...
# ------------------------------------------------------------------------------
gh_compo_interface = gh_compo_io.GHCompo_PHNavV1GetApertures(
    IGH,
    _project_number,
    _version,
