...
EM January 17, 2025
    Args:
        ...
        _column_name: (str) The name of the column to get the data for.

    Returns:
        column_data_: (list) A list of all of the Column Data found in the specified SQL file.
"""
...
ghenv.Component.Name = "HBPH+ - SQL Get Column Data"
DEV = honeybee_ph_plus_rhino._component_info_.set_component_params(ghenv, dev=False)
if DEV:
    reload(gh_io)
    from honeybee_ph_plus_rhino.gh_compo_io.hb_tools import sql_get_column_data as gh_compo_io
    reload(gh_compo_io)
...
# ------------------------------------------------------------------------------
gh_compo_interface = gh_compo_io.GHCompo_SQLGetColumnData(
    IGH, _sql_file, _table_name, _column_name
)
column_data_ = gh_compo_interface.run()
